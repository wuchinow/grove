...
// error, so the client can treat "no session" as a normal state rather than
// a failure.
export async function GET() {
  const c = cfg();
  if (!c) return Response.json({ error: "Server is missing Supabase settings." }, { status: 500 });
  const [me, settings] = await Promise.all([currentStudent(c), fetchPublicSettings(c)]);
  if (!me) return Response.json({ student: null, settings });
